...
```
- we refer to designs using the `~` operator, 

```{r echo=T}
des_obj <- DESeqDataSetFromTximport(  
  txi = txi,
...
## Exploring data
- Once we load data into R, the next step is to explore the data.
...
## PCA in DESeq2
- generating PCA plots is built into DESeq2
```{r}
