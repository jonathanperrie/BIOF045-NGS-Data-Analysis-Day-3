...
## Beyond Bulk RNA-seq
- I'll cover some other types of RNA-seq data
- Unfortunately we won't have time to go through code for these types of RNA-seq 
- However, the general workflow is the same across all types of RNA-seq
- Mapping > quantification  > QC > analysis
- Bulk RNA-seq data refers to RNA generated from bulk tissue/ large quantities of RNA
...
<div class="col">
```{r, echo = F}
include_graphics('../src/miRNAseq.png')
```

</div>

<div class="col">
- microRNA(miRNA) are small RNAs that regulate mRNA expression, through RNA interference
- Sequencing works the exact same as bulk RNA-seq, except there is a size selection step for smaller fragments 
</div>
</div>
...
- individual cells are pipetted into wells(through serial dilution), where each well has reagents for RNA-seq library prep. 
- cells are the lysed and sequenced. 
- this typically yields 100-1000 individual cells cells 
- Functions essentially the same as bulk RNA-seq, in that whole transcripts are amplified
</div>
</div>
...
- individual cells are encapsulated in oil coated beads through a microfluidics system
- RNA-seq library preparation occurs within these droplets
- only the 3' end of transcripts are sequenced, so can only get gene level expression for about 2000 total genes
- cells are the lysed and sequenced. 
- this typically yields 5000-15000 individual cells 
</div>
</div>
...
<div class="col">
```{r, echo = F}
include_graphics('../src/nanopore.png')
```

</div>
<div class="col">
- sometimes referred to third generation RNA-sequencing 
- whole RNA transcripts(300bp - 50000bp) can be directly sequenced
- Transcripts are fed through a helicase enzyme embedded in an electrically charged membrane
- as the DNA passes through the enzyme, the charge changes across the membrane, which can be used to determine the nucleotide sequence
- can be done with cDNA or direct RNA
- useful for identifying gene isoforms and RNA modifications 
</div>
</div>
